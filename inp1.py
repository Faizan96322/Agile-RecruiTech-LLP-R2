...
merged_df['Team Name'] = merged_df['Team Name'].str.strip()

# Group by the team name and calculate the average of total_statements
out1_df = merged_df.groupby('Team Name').agg({'total_statements': 'mean','total_reasons': 'mean'})
out1_df = out1_df.rename(columns={'total_statements':'average_statements','total_reasons':'average_reasons'})
out1_df = out1_df.sort_values(by='average_statements', ascending=False)
out1_df = out1_df.reset_index()
out1_df.index += 1
# Print the grouped DataFrame
print(out1_df)
out1_df.to_excel('output1.xlsx', index=False)